...
import { BookOpen, Calendar, Clock, Users, Plus, Search, Filter, Edit, Trash2 } from 'lucide-react';

export default function StemCourseManagementPage() {
  return (
    <motion.div
    ...
    >
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-800">STEM课程管理</h2>
          <p className="text-sm text-gray-500 mt-1">管理第二课堂兴趣课程，支持课后和周末时段安排</p>
        </div>
        ...
        <table className="w-full text-left">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase">时间段</th>
              <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase">适合年级</th>
              <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase">报名情况</th>
          ...
          <tbody className="divide-y divide-gray-100">
            {[
              { name: 'Arduino 创意编程', type: '硬件编程', time: '周一 15:30-17:00', grade: '初一-初二', enrollment: '18/20', status: '进行中' },
              { name: 'Python 游戏开发', type: '软件编程', time: '周三 15:30-17:00', grade: '初二-初三', enrollment: '15/15', status: '已满员' },
              { name: '3D 打印设计', type: '创客制作', time: '周五 15:30-17:00', grade: '初一-初三', enrollment: '12/16', status: '进行中' },
              { name: '机器人竞赛培训', type: '竞赛辅导', time: '周六 09:00-11:00', grade: '初二-高三', enrollment: '20/20', status: '已满员' },
              { name: 'AI 视觉识别入门', type: '人工智能', time: '周日 14:00-16:00', grade: '高一-高二', enrollment: '8/12', status: '待开课' },
            ].map((course, i) => (
              <tr key={i} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-6 py-4 text-sm text-gray-600">{course.time}</td>
                  ...
                  <div className="flex items-center gap-2">
                    <div className="w-16 bg-gray-200 rounded-full h-2">
                      <div 
                        ...
                        style={{ width: `${(parseInt(course.enrollment.split('/')[0]) / parseInt(course.enrollment.split('/')[1])) * 100}%` }}
                      ></div>
                    </div>
                    <span className="text-xs text-gray-600">{course.enrollment}</span>
                  </div>
                </td>
                <td className="px-6 py-4">
                  <span className={`px-2 py-1 rounded-full text-xs ${
                    course.status === '进行中' ? 'bg-green-50 text-green-600' :
                    course.status === '已满员' ? 'bg-red-50 text-red-600' :
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {[
            { day: '周一', time: '15:30-17:00', course: 'Arduino 创意编程', room: 'STEM-101', teacher: '李老师' },
            { day: '周二', time: '15:30-17:00', course: 'Python 游戏开发', room: 'STEM-102', teacher: '王老师' },
            ...
            { day: '周六', time: '09:00-11:00', course: '机器人竞赛培训', room: 'STEM-103', teacher: '赵老师' },
            { day: '周日', time: '14:00-16:00', course: 'AI 视觉识别入门', room: 'STEM-102', teacher: '孙老师' },
          ].map((schedule, i) => (
